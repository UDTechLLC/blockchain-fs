	...
	for {
		...
		if err == syscall.EOPNOTSUPP {
			...
			preallocWarn.Do(func() {
				...
				tlog.Warn("Warning: The underlying filesystem " +
...
func Openat(dirfd int, path string, flags int, mode uint32) (fd int, err error) {
	...
	if !(flags&syscall.O_CREAT != 0 && flags&syscall.O_EXCL != 0) && flags&syscall.O_NOFOLLOW == 0 {
		...
		tlog.Warn("Openat: adding missing O_NOFOLLOW flag")
...
func Fchmodat(dirfd int, path string, mode uint32, flags int) (err error) {
	...
	if flags&unix.AT_SYMLINK_NOFOLLOW == 0 {
		...
		tlog.Warn("Fchmodat: adding missing AT_SYMLINK_NOFOLLOW flag")
...
func Fchownat(dirfd int, path string, uid int, gid int, flags int) (err error) {
	...
	if flags&unix.AT_SYMLINK_NOFOLLOW == 0 {
		...
		tlog.Warn("Fchownat: adding missing AT_SYMLINK_NOFOLLOW flag")
		flags |= unix.AT_SYMLINK_NOFOLLOW
	}
...
func Fstatat(dirfd int, path string, stat *unix.Stat_t, flags int) (err error) {
	...
	if flags&unix.AT_SYMLINK_NOFOLLOW == 0 {
		...
		tlog.Warn("Fstatat: adding missing AT_SYMLINK_NOFOLLOW flag")
		flags |= unix.AT_SYMLINK_NOFOLLOW
	}
